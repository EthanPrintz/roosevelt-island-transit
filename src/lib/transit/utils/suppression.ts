 ...
 * Calculates the maximum real-time prediction timestamp (T_max_live) for each stop & direction.
...
export function suppressGhostSchedules<T extends TransitDeparture>(departures: T[]): T[] {
	...
	// 1. Identify maximum live real-time prediction timestamp per stop & direction
	for (const dep of departures) {
		if (!dep.isRealtime) continue;
		const timeMs = new Date(dep.predictedTime || dep.scheduledTime).getTime();
		const stopKey = `${dep.stopId || dep.stopName || 'all'}:${dep.direction}`;
